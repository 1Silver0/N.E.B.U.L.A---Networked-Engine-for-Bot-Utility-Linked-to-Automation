...
class Income(commands.Cog):
    ...
    @commands.command()
    async def income(self, ctx, category: str, price: float):
        async with aiosqlite.connect("YOUR_STORAGE_FILE") as db:
            ...
            await db.execute("INSERT OR REPLACE INTO memory (user, category, price, type) VALUES (?, ?, ?, 'income')", (str(ctx.author.id), category, price))
            await db.commit()

        file = discord.File("NEBULA/NebulaV1_2/SUCCESS.png", filename="SUCCESS.png")
        embed = discord.Embed(description=f"**Income: {category}** saved at the price of **{price}**", color=discord.Color.green())
